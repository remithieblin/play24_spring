...
class SpringApplicationLoader_draft extends ApplicationLoader {
    ...
    if (!isSimpleTypeAlias) {
      ...
      binding.target match {
        ...
        case Some(ConstructionTarget(clazz)) =>
          ...
          SpringApplicationLoader_draft.maybeSetScope(beanDef, clazz.asInstanceOf[Class[_]])
      ...
      binding.scope match {
        ...
        case Some(scope) =>
          SpringApplicationLoader_draft.setScope(beanDef, scope)
      }
...
private object SpringApplicationLoader_draft {
